...
%% import data
data = importdata('variance_MPU9250.txt',' ',1); % import the variance of sensor data. Angles are in degree.

%% symbol and data in ROM
s=tf('s');
Devi_yaw = sqrt(data.data(1,1)/180*pi);
Devi_gz = sqrt(data.data(1,9)/180*pi);
...
thrust_M=0.5*(l/2)*(3/4); % actual moment produced by thruster (to compenstate the friction)
...
%%%%%%%%%%%%%%% sensor %%%%%%%%%%%%%%
K_angle=0.3; %feedback gain for angle position %0.2
K_angu_v=0.6; %feedback gain for angular velocity %0.4

%%%%%%%% controller %%%%%%%%
uplimit=thrust_M_design;
deadband=uplimit*0.8; %3/180*pi*K_angle; % (acceptable angle error + acceptable angu_V error)/180*pi*K_angle
...
%     angu_v_sensorGet_thrust=awgn(angu_v_response_thrust(1,(sensor_point-1)*scale_sensor2data+1),6,'measured');
    ...
    angle_sensorGet_thrust=angle_response_thrust(1,(sensor_point-1)*scale_sensor2data+1)+ random('norm',0,Devi_yaw);
    angu_v_sensorGet_thrust=angu_v_response_thrust(1,(sensor_point-1)*scale_sensor2data+1)+ random('norm',0,Devi_gz);
...
%     end
    ...
    %%%%%%%%%%%%%%%%% pulse modulator %%%%%%%%%%%%%%%%%
    if sensor_point==1 %initialized conditions for the pulse modulation algorithm
...
figure,
subplot(3,1,1)
plot(testing_period,angle_response_thrust/pi*180,'b',testing_period,disturbance_impulse,'g',testing_period,disturbance_impulse,'g',testing_period,angle_response/pi*180,'r'), grid on, title('angle (degree)'), %,testing_period,disturbance_impulse,'g',testing_period,angle_response/pi*180,'r'
% dim = [0.4 0.8 0.1 0.1];
% C=num2str(K_angle);
% note='K-angle =';
% str=strcat(note,C);
